...
class Corpus(object):)
    def __init__(self, documents, ratio=1.0, shuffle=False):
        self.documents = cp.get_corpus_by_tag(*tag_list)
    ...
    def generate_to_unk_dict(self, unk_threshold, unk_oov_ratio, trans_prob, known_unk_dict={}):
        ...
        to_unk_dict = self.generate_to_unk_dict(token_freq_dist, vocabulary_size
                                                unk_threshold, unk_oov_ratio, trans_prob)
        to_unk_dict.update(known_unk_dict)
        return to_unk_dict
    ...
    def generate_to_unk_dict(self,token_freq_dist, vocabulary_size, unk_threshold, unk_oov_ratio, trans_prob):
        unk_count = 0
        to_unk_dict = {}
        regex_not_to_unk = re.compile(r'^(?!https://|http://|#|@).*$')
        for k,v in random.sample(token_freq_dist.items(), vocabulary_size):
            if regex_not_to_unk.match(v) and v <= unk_threshold and \
                    unk_count <= unk_oov_ratio * vocabulary_size and \
                    random.uniform(0,1) < trans_prob:
                to_unk_dict.update({v:UNK})
                unk_count += 1
            elif v[0]=='@':
                to_unk_dict.update({v:NAME})
            elif v[0]=='#':
                to_unk_dict.update({v:HASHTAG})
            elif re.match(r'^(http://|https://).*$', v):
                to_unk_dict.update({v:URL})
